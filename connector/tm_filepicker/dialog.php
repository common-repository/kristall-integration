...
defined( 'ABSPATH' ) || exit;
		...
		$getKeyA = sanitize_text_field($_GET['akey']);
		
		if (USE_ACCESS_KEYS == true){
			if (!isset($getKeyA, $config['access_keys']) || empty($config['access_keys'])){
				die('Access Denied!');
			}

			$getKeyA = strip_tags(preg_replace("/[^a-zA-Z0-9\._-]/", '', $getKeyA));

			if (!in_array($getKeyA, $config['access_keys'])){
				die('Access Denied!');
			}
		}
		
		$_SESSION['RF']["verify"] = "kristallConnectTmflp";
		...
		if (isset($postSubmit)) {
			...
			$available_languages = include $path_lib . 'lang/languages.php';
			
			$getLang = sanitize_text_field($_GET['lang']);

			list($preferred_language) = array_values(array_filter(array(
				isset($getLang) ? $getLang : null,
		...
		include $path_lib . 'include/utils.php';
		
		$subdir_path = '';
		
		$getFldr = sanitize_text_field($_GET['fldr']);

		if (isset($getFldr) && !empty($getFldr)) {
			$subdir_path = rawurldecode(trim(strip_tags($getFldr), "/"));
		...
		$cur_dir_url		= $config['thumbs_base_url'].$rfm_subfolder.$subdir;
		
		$cur_dir_thumb		= $cur_dir . $config['thumbs_upload_dir'].$rfm_subfolder.$subdir;
		$thumbs_path		= $config['upload_dir'] . $config['thumbs_base_path'] . $rfm_subfolder.$subdir;
		$thumbs_base_url    = $config['thumbs_base_url'] . $config['thumbs_base_path'].$rfm_subfolder.$subdir;
		...
		if (!$ftp) {
			...
			if (!is_dir($cur_dir_thumb)) {
				create_folder(FALSE, $cur_dir_thumb, $ftp, $config);
			}
		}
		
		$multiple = null;
		
		$getMultiple = sanitize_text_field($_GET['multiple']);
		
		if (isset($getMultiple)) {
			if ($getMultiple == 1) {
				$multiple = 1;
				$config['multiple_selection'] = true;
				$config['multiple_selection_action_button'] = true;
			} elseif ($getMultiple == 0) {
				$multiple = 0;
		...
		$getCallback = sanitize_text_field($_GET['callback']);

		if (isset($getCallback)) {
			$callback = strip_tags($getCallback);
		...
		$popup = isset($getPopup) ? strip_tags($getPopup) : 0;
		//Sanitize popup
		$popup = !!$popup;
		
		$getCrossdomain = sanitize_text_field($_GET['crossdomain']);

		$crossdomain = isset($getCrossdomain) ? strip_tags($getCrossdomain) : 0;
		...
		$getView = (int) sanitize_text_field($_GET['view']);
		...
		$getFilter = sanitize_text_field($_GET["filter"]);
		...
		$getSortBy = sanitize_text_field($_GET["sort_by"]);

		if (isset($getSortBy)) {
			$sort_by = $_SESSION['RF']['sort_by'] = $getSortBy;
		} else {
		...
		$getDescending = sanitize_text_field($_GET["descending"]);

		if (isset($getDescending)) {
			$descending = $_SESSION['RF']['descending'] = $getDescending == 1;
		} else {
			$descending = $_SESSION['RF']['descending'];
		}

		$boolarray = array(false => 'false', true => 'true');
		$getRelativeUrl = sanitize_text_field($_GET['relative_url']);
		$return_relative_url = isset($getRelativeUrl) && $getRelativeUrl == "1" ? true : false;
		
		$getType = sanitize_text_field($_GET['type']);
		...
		$extensions = null;
		$getExtensions = $_GET['extensions'];
		
		if (isset($getExtensions)) {
			$extensions = json_decode(urldecode($getExtensions));
			$ext_tmp = array();
		...
		$getEditor = sanitize_text_field($_GET['editor']);
		$getFieldId = sanitize_text_field($_GET['field_id']);

		if (isset($getEditor)) {
			$editor = strip_tags($getEditor);
		} else {
			$editor = $getType == 0 ? null : 'tinymce';
		}

		$field_id = isset($getFieldId) ? fix_get_params($getFieldId) : null;
		$type_param = fix_get_params($getType);
		$apply = null;
		...
		$get_params = array(
			...
			'akey'          => (isset($getKeyA) && $getKeyA != '' ? $getKeyA : 'key')
		);
		
		$getCKEditorFuncNum = sanitize_text_field($_GET['CKEditorFuncNum']);
		$getCKEditor = sanitize_text_field($_GET['CKEditor']);
		
		if ($getCKEditorFuncNum) {
			$get_params['CKEditorFuncNum'] = $getCKEditorFuncNum;
			$get_params['CKEditor'] = (isset($getCKEditor) ? $getCKEditor : '');
		}
...
<html>
	<head>
        ...
        <title>Kristall File Sub Manager</title>
        <link rel="shortcut icon" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/images/connector_sub/ico/favicon.ico">
        <!-- CSS to style the file input field as button and adjust the Bootstrap progress bars -->
        <link rel="stylesheet" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/jquery.fileupload.css">
        <link rel="stylesheet" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/jquery.fileupload-ui.css">
        <!-- CSS adjustments for browsers with JavaScript disabled -->
        <noscript><link rel="stylesheet" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/jquery.fileupload-noscript.css"></noscript>
        <noscript><link rel="stylesheet" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/jquery.fileupload-ui-noscript.css"></noscript>
        <link rel="stylesheet" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/jplayer.blue.monday.min.css" />
        <link rel="stylesheet" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/tui-image-editor.css">
        <link href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/style.css" rel="stylesheet" type="text/css" />
        <!--[if lt IE 8]>
        <style>
        ...
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/base.min.js"></script>
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/plugins.js?v=<?php echo $version; ?>"></script>
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/jquery.jplayer.min.js"></script>
        <link type="text/css" href="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/css/connector_sub/tui-color-picker.css" rel="stylesheet">
        <script type="text/javascript" src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/fabric.js"></script>
        <script type="text/javascript" src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/tui-code-snippet.min.js"></script>
        <script type="text/javascript" src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/FileSaver.min.js"></script>
        <script type="text/javascript" src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/tui-color-picker.js"></script>
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/tui-image-editor.min.js"></script>
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/modernizr.custom.js"></script>

        <!-- Le HTML5 shim, for IE6-8 support of HTML5 elements -->
        <!--[if lt IE 9]>
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/html5shiv.js"></script>
        <![endif]-->
        ...
        <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/include.js?v=<?php echo $version; ?>"></script>
	</head>
	<body>
    <!-- The Templates plugin is included to render the upload/download listings -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/tmpl.min.js"></script>
    <!-- The Load Image plugin is included for the preview images and image resizing functionality -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/load-image.all.min.js"></script>
    <!-- The Canvas to Blob plugin is included for image resizing functionality -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/vendor/canvas-to-blob.min.js"></script>
    <!-- The Iframe Transport is required for browsers without support for XHR file uploads -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.iframe-transport.js"></script>
    <!-- The basic File Upload plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload.js"></script>
    <!-- The File Upload processing plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload-process.js"></script>
    <!-- The File Upload image preview & resize plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload-image.js"></script>
    <!-- The File Upload audio preview plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload-audio.js"></script>
    <!-- The File Upload video preview plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload-video.js"></script>
    <!-- The File Upload validation plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload-validate.js"></script>
    <!-- The File Upload user interface plugin -->
    <script src="<?php echo KRISTALL_PLUGIN_URL; ?>/assets/js/connector_sub/jquery.fileupload-ui.js"></script>
    ...
    <input type="hidden" id="thumbs_base_url" value="<?php echo $config['thumbs_base_url'];?>" />
    <input type="hidden" id="cur_dir" value="<?php echo $cur_dir_url;?>" />
	...
	foreach ($files as $k => $file) {
		if ($ftp) {
			...
			$sorted[$k] = array(
				'is_dir' => $is_dir,
				'file' => $file['name'],
				'file_lcase' => mb_strtolower($file['name']),
				'date' => $date,
				'size' => $size,
			...
			if ($file != "." && $file != "..") {
				if (is_dir($config['current_path'] . $rfm_subfolder . $subdir . $file)) {
					...
					$sorted[$k] = array(
						...
						'size' => $size,
						'permissions' => '',
						'extension' => mb_strtolower($file_ext)
					);
				}
			}
		}
    ...
    <div class="navbar-inner">
        ...
        <div class="filters">
            ...
            <div class="span6 entire types">
                ...
                <?php if(sanitize_text_field($_GET['type'])!=1 && sanitize_text_field($_GET['type'])!=3 && $config['show_filter_buttons']){ ?>
                ...
                <label id="ff-item-type-5" title="<?php echo trans('Music');?>" for="select-type-5" class="tip btn ff-label-type-5"><i class="icon-music"></i></label>
                    <?php } ?>
                <?php } ?>
                <input accesskey="f" type="text" class="filter-input <?php echo ((sanitize_text_field($_GET['type'])!=1 && sanitize_text_field($_GET['type'])!=3) ? '' : 'filter-input-notype');?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(trans('Text_filter'));?>..." value="<?php echo $filter;?>"/><?php if($n_files>$config['file_number_limit_js']){ ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>

                <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
                <label id="ff-item-type-all" title="<?php echo trans('All');?>" <?php if(sanitize_text_field($_GET['type'])==1 || sanitize_text_field($_GET['type'])==3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><?php echo trans('All');?></label>
...
<!-- header div end -->
    ...
    <div class="row-fluid">
    <?php
    $link = "/wp-admin/admin-ajax.php?action=krConnectTmflp&" . $get_params;
    ?>
    <ul class="breadcrumb">
    ...
    <?php } ?>
    <li class="pull-right"><a id="refresh" class="btn-small" href="/wp-admin/admin-ajax.php?action=krConnectTmflp&<?php echo $get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
	...
	<div class="span12">
        ...
        foreach ($files as $file_array) {
            ...
            if($file!='..'){
                if(!$ftp && !file_exists($cur_dir_thumb.$file)){
                    create_folder(false,$cur_dir_thumb.$file,$ftp,$config);
                }
            }
            ...
            ?>
                ...
                <?php if($file==".."){ ?>
                    ...
                    <input type="hidden" class="path_thumb" value="<?php echo dirname($cur_dir_thumb)."/";?>"/>
                <?php } ?>
                <a class="folder-link" href="/wp-admin/admin-ajax.php?action=krConnectTmflp&<?php echo $get_params.rawurlencode($src)."&".($callback?'callback='.$callback."&":'').uniqid() ?>">
                    <div class="img-precontainer">
                            <div class="img-container directory"><span></span>
                            <img class="directory-img" data-src="/wp-content/plugins/kristall-integration/connector/assets/images/connector_sub/<?php echo $config['icon_theme'];?>/folder<?php if($file==".."){ echo "_back"; }?>.png" />
                            </div>
                    </div>
            ...
            <?php }else{ ?>
                    ...
                    <div class="box">
                    <h4 class="<?php if($config['ellipsis_title_after_first_row']){ echo "ellipsis"; } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="/wp-admin/admin-ajax.php?action=krConnectTmflp&<?php echo $get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file;?></a></h4>
                    </div>
            ...
            foreach ($files as $nu=>$file_array) {
                ...
                if(!$ftp){
                    $file_path=$config['current_path'].$rfm_subfolder.$subdir.$file;
                    $file_url=$config['thumbs_base_url'].$rfm_subfolder.$subdir.$file;
                ...
                if(in_array($file_array['extension'], $config['ext_img'])){
                    ...
                    if($ftp){
						...
						$src_thumb = $thumbs_base_url. $file;
                ...
                if($src_thumb==""){
                    $no_thumb=true;
                    if(file_exists('/wp-content/plugins/kristall-integration/connector/assets/images/connector_sub/'.$config['icon_theme'].'/'.$file_array['extension'].".jpg")){
                        $src_thumb ='/wp-content/plugins/kristall-integration/connector/assets/images/connector_sub/'.$config['icon_theme'].'/'.$file_array['extension'].".jpg";
                    }else{
                        $src_thumb = "/wp-content/plugins/kristall-integration/connector/assets/images/connector_sub/".$config['icon_theme']."/default.jpg";
                    }
                    $is_icon_thumb=true;
                }
                if($mini_src==""){
                ...
                if((!(sanitize_text_field($_GET['type'])==1 && !$is_img) && !((sanitize_text_field($_GET['type'])==3 && !$is_video) && (sanitize_text_field($_GET['type'])==3 && !$is_audio))) && $class_ext>0){
?>
            ...
            <?php if($config['multiple_selection']){ ?><div class="selector">
                ...
                <figcaption>
                    <form action="/wp-admin/admin-ajax.php?action=krConnectTmflpFD" method="post" class="download-form" id="form<?php echo $nu;?>">
                    ...
                    title="<?php echo trans('Preview')?>" data-url="/wp-admin/admin-ajax.php?action=krConnectTmflpCalls&maction=media_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
                    <?php }elseif(in_array($file_array['extension'],$config['cad_exts'])){ ?>
                    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="/wp-admin/admin-ajax.php?action=krConnectTmflpCalls&maction=cad_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
                    <?php }elseif($config['preview_text_files'] && in_array($file_array['extension'],$config['previewable_text_file_exts'])){ ?>
                    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="/wp-admin/admin-ajax.php?action=krConnectTmflpCalls&maction=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
                    href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
                    <?php }elseif($config['googledoc_enabled'] && in_array($file_array['extension'],$config['googledoc_file_exts'])){ ?>
                    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="/wp-admin/admin-ajax.php?action=krConnectTmflpCalls&maction=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
...
<script>
    ...
    <div id="loading_container" style="display:none;">
        ...
        <img id="loading_animation" src="/wp-content/plugins/kristall-integration/connector/assets/images/connector_sub/storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%">
    </div>
    <!-- loading div end -->
    ...
    <?php if ( $config['tui_active'] ) { ?>
        ...
        if (image_editor) { 
            ...
            function saveTUI()
            {
                ...
                $.ajax({
                    type: "POST",
                    url: "/wp-admin/admin-ajax.php?action=krConnectTmflpCalls&maction=save_img",
