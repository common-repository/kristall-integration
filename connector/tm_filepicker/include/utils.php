...
if ( ! function_exists('trans'))
{
	...
	*/
	function trans($var)
	{
		global $kristall_fm_lang_vars;

		return (array_key_exists($var, $kristall_fm_lang_vars)) ? $kristall_fm_lang_vars[ $var ] : $var;
	}
	
	$path_lib = KRISTALL_PLUGIN_DIR . '/lib/tm_filepicker/';

	// language
	if ( ! isset($_SESSION['RF']['language'])
		|| file_exists($path_lib.'lang/' . basename($_SESSION['RF']['language']) . '.php') === false
		|| ! is_readable($path_lib.'lang/' . basename($_SESSION['RF']['language']) . '.php')
	)
	{
	...
	if(file_exists($path_lib.'lang/' . $lang . '.php')){
		$GLOBALS['kristall_fm_lang_vars'] = include $path_lib.'lang/' . $lang . '.php';
	}else{
		$GLOBALS['kristall_fm_lang_vars'] = include $path_lib.'../lang/' . $lang . '.php';
	}

	if ( ! is_array($GLOBALS['kristall_fm_lang_vars']))
	{
		$GLOBALS['kristall_fm_lang_vars'] = array();
	}
}
...
if (!function_exists('create_img')) {
	function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop",$config = array()){
		...
		if(file_exists($imgfile) || strpos($imgfile,'http')===0){
			...
			{
				require_once(KRISTALL_PLUGIN_DIR.'/lib/tm_filepicker/include/ImageCropper.php');
				try{
					$magicianObj = new ImageCropper($imgfile);
