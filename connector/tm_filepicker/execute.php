...
$path = $base . sanitize_text_field($_POST['path']);
$cycle = true;
$max_cycles = 50;
$i = 0;
...
$info = pathinfo($path);
if (isset($info['extension']) && !(isset($_GET['maction']) && $_GET['maction'] == 'delete_folder') &&
	!check_extension($info['extension'], $config)
	&& $_GET['maction'] != 'create_file') {
...
if (isset($_GET['maction'])) {
	switch ($_GET['maction']) {
		...
		case 'paste_clipboard':
			...
			$action = sanitize_text_field($_SESSION['RF']['clipboard_action']);
			$data = sanitize_text_field($_SESSION['RF']['clipboard']);
